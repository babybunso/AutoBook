...
/**
 * Modified CodeIgniter Template Class
 *
 * This modification moves the template folders from application/views to public/themes
 *
 * @package   CodeIgniter
 * @author     
 * @subpackage  Libraries
...
class Template {
   ...
   {
      if (file_exists(FCPATH .'themes/'. $filename) or file_exists(FCPATH .'themes/'. $filename . EXT))
      {
         $this->master = $filename;
      }
      else
      {
         show_error('The filename provided does not exist in <strong>'. FCPATH .'themes</strong>. Remember to include the extension if other than ".php"');
      }
   }
   // function set_master_template($filename)
   // {
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template']) 
         && (file_exists(FCPATH .'themes/'. $props['template']) or file_exists(FCPATH .'themes/'. $props['template'] . EXT)))
      {
      ...
      else 
      {
         ...
         show_error('Either you have not provided a master template or the one provided does not exist in <strong>'. FCPATH .'themes</strong>. Remember to include the extension if other than ".php"');
      }
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      if (count($args) > 1)
      {
         ...
         {
            if (file_exists(FCPATH .'themes/'. $suggestion . EXT) or file_exists(FCPATH .'themes/'. $suggestion))
            {
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            $output = $this->CI->load->ext_view('themes', $this->master, $this->output, $buffer);
         }
      }
